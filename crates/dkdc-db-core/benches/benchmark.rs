...
fn bench_scan_full(c: &mut Criterion) {
    ...
    for size in [1_000, 10_000, 100_000, 500_000] {
        ...
        group.bench_with_input(BenchmarkId::new("turso", size), &size, |b, _| {
            b.iter(|| {
                rt.block_on(async {
                    db.query_turso("SELECT * FROM sales").await.unwrap();
                })
            })
        });
    }
...
fn bench_scan_projected(c: &mut Criterion) {
    ...
    for size in [1_000, 10_000, 100_000, 500_000] {
        ...
        group.bench_with_input(BenchmarkId::new("turso", size), &size, |b, _| {
            b.iter(|| {
                rt.block_on(async {
                    db.query_turso("SELECT region, amount FROM sales")
                        .await
                        .unwrap();
                })
            })
...
fn bench_agg_simple(c: &mut Criterion) {
    ...
    for size in [1_000, 10_000, 100_000, 500_000] {
        ...
        group.bench_with_input(BenchmarkId::new("turso", size), &size, |b, _| {
            b.iter(|| rt.block_on(async { db.query_turso(sql).await.unwrap() }))
        });
    }
    group.finish();
}
